...
def saveRect(request):
	if request.method == 'POST':
		...
		try:
			...
			print(duration)
			models.Rectangle.objects.create(time=params['time'], name=params['name'], x1=params['x1'], y1=params['y1'], x2=params['x2'], y2=params['y2'], duration=duration)
...
def saveDuration(request):
	if request.method == 'POST':
		...
		try:
			...
			# username = models.Username.get
			duration = models.Duration.objects.create(time=params['time'], name=params['name'], consumingtime=params['consumingtime'])
