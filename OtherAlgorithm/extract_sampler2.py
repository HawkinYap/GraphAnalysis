...
def saveGraph(G, Gs, filename, iter, sample_type, rate):
    class_nodes = []
    for node in G.nodes():
        if node in Gs.nodes():
    ...
    # test csv
    classfile_path = "samplingResult/G/two-step/{}/{}_{}_{}_{}_node.csv".format(filename, sample_type, filename, rate, iter)
    orig_edgefile_path = "samplingResult/G/two-step/{}/{}_{}_{}_{}_edge.csv".format(filename, sample_type, filename, rate, iter)

    title = ['ID', 'cluster']
    test = pd.DataFrame(data=class_nodes, columns=title)
    test.to_csv(classfile_path, index=None, header=True)

    title = ['Source', 'Target', 'cluster']
    ...
    for ii in sample_types:
        ...
        for jj in iter:
            for kk in count:
                ...
                isDirect = False
                G, Gs = loadData(path1, path2, isDirect)
