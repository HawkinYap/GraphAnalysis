...
def bridgeMerge(inner, s=0):
    ...
    if s == 1:
        ...
        for i in two_tuple:
            ...
            if count_node[i[1]]>2:
                parachute.add(i[1])
        parachute_sort = sorted(parachute, key=lambda d: count_node[d], reverse=True)
        print(parachute_sort)
...
def Articulation_Points_and_Bridge(G, anomaly_total):
    ...
    # print('tie', sort_inner)
    print('rim', sort_outer)
...
def loadData(path1, path2, isDirect):
    ...
    # for item in reader1:
    #     if int(item[1]) == 1:
    #         nodes1.append(int(item[0]))
    #     if int(item[1]) == 2:
    #         nodes2.append(int(item[0]))
    for item in reader1:
        nodes1.append(int(item[0]))
    f.close()
    if isDirect:
...
# data processing
def dataTest():
    # path1 = "as/as_TIES_0.3_node.csv"
    # path2 = "as/as_TIES_0.3_edge.csv"
