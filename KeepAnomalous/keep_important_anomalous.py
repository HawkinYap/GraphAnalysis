...
def Extract_Global_High_Neighbor(G, heigh_neighbour, s=0):
    ...
    for node in hubs:
        index += 1
        if s == 0:
            G.node[node]['corerank'] = index
        if s == 1:
            G.node[node]['coreranks'] = index
...
def Extract_Star(G, threshold, s=0):
    ...
    for n in star_rank:
        index += 1
        if s == 0:
            G.node[node]['starrank'] = index
        if s == 1:
            G.node[node]['starranks'] = index
        if G.node[n]['star'] == 0:
            G.node[n]['star'] = 1
            new_node += 1
        else:
...
def Articulation_Points(G, s=0):
    ...
    for node in arti:
        index += 1
        if s == 0:
            G.node[node]['artirank'] = index
        if s == 1:
            G.node[node]['artiranks'] = index
